...
__global__ void flashattn_kernel_v5(
    ...
    for (int j = 0; j <= i; ++j) {
        ...
        for (int k_row = 0; k_row < Br; ++k_row) {
            ...
            K_smem[k_row * (d + 1) + k_col] = K[batch_head_offset + k_idx];
        }
        __syncthreads();
        ...
        for (int k = 0; k < d; ++k) {
            ...
            for (int nn = 0; nn < TN; ++nn)
                K_reg[nn] = K_smem[(tile_thread_col_id * TN + nn) * (d + 1) + k];
        ...
        for (int mm = 0; mm < TM; ++mm) {
            ...
            for (int nn = 0; nn < TN; ++nn) {
                ...
                S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) + tile_thread_col_id * TN + nn] = s_val;
            }
        }
        __syncthreads();
        ...
        for (int mm = 0; mm < TM; ++mm) {
            ...
            for (int k = 0; k < Bc; ++k)
                m_ij = fmaxf(m_ij, S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) + k]);
            m[mm] = m_ij;
        }

        for (int mm = 0; mm < TM; ++mm) {
            for (int nn = 0; nn < TN; ++nn) {
                float p = __expf(S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) +
                                             tile_thread_col_id * TN + nn] - m[mm]);
                S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) +
        ...
        for (int mm = 0; mm < TM; ++mm) {
            ...
            for (int k = 0; k < Bc; ++k)
                l_ij += S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) + k];
            l[mm] = l_ij;
        }
        ...
        for (int k = 0; k < Bc; ++k) {
            for (int mm = 0; mm < TM; ++mm) {
                Q_reg[mm] = S_ij_smem[(tile_thread_row_id * TM + mm) * (Bc + 1) + k]; //Save some registers, don't know the full calcs yet
            }
...
void flashattn_v5(const float *Q, const float *K, const float *V, float *O,
                float *l, float *m, 
    ...
    flashattn_kernel_v5<BLOCK_SIZE, Br, Bc, Bk, TM, TN, TK><<<grid_dim, block_dim>>>(
