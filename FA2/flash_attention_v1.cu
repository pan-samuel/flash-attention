    ...
    int seqlen_q)
{
    ...
    int bid = blockIdx.y; //batch_head_id
    ...
    nv_bfloat16* K_smem = Q_smem;
    nv_bfloat16* V_smem = K_smem + (BLOCK_N * d) * sizeof(nv_bfloat16);


    // MMA configuration for SM80_16x8x16_F32F16F16F32_TN
    ...
    // Derived constants for better readability
    constexpr int D_TILES = d / MMA_N;  // Number of tiles in d dimension
    constexpr int N_TILES = BLOCK_N / MMA_N;  // Number of tiles in N dimension
    constexpr int K_TILES = d / MMA_K;  // Number of tiles in K dimension
    ...
    uint32_t O_reg[D_TILES][NUM_REG_A] = {0.f};

    // Initialize max scores to negative infinity for proper max reduction
    float scores_max[2] = {-INFINITY, -INFINITY};
    // Initialize sum scores to zero for proper accumulation
    ...
    for (int k_tile = 0; k_tile <= m_block; ++k_tile) {
        float S_reg[BLOCK_N / MMA_N][NUM_REG_C] = {0.0f};
        ...
        // Apply causal masking (lower triangular attention)
        ...
        for (int mma_tile_id_n = 0; mma_tile_id_n < D_TILES; ++mma_tile_id_n) 
            for (int mma_tile_id_k = 0; mma_tile_id_k < NK_TILES; ++mma_tile_id_k)
                mma(S_reg[mma_tile_id_k], V_reg[mma_tile_id_n][mma_tile_id_k], O_reg[mma_tile_id_k]);

    }

    // Normalize output by sum to complete softmax computation
    for (int mma_tile_id_k = 0; mma_tile_id_k < D_TILES; ++mma_tile_id_k) {
